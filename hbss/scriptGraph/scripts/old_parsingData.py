...
WALL_DIST = 5 #for open environment, twice the GRID_DISCRETIZATION
...
if __name__ == '__main__':
	...
	environment_yaml_path = os.getcwd() + '/envs/' + gflags.FLAGS.environment + '.yaml'
	...
	for i in range(0,len(POINTS)):
		old_pos = (G_E.vs[POINTS[i]]['x_coord'], G_E.vs[POINTS[i]]['y_coord'])
		#print pos
		found = False
		changed = False
		large = False
		while not found:
			if all_free(int(G_E.vs[POINTS[i]]['y_coord']), int(G_E.vs[POINTS[i]]['x_coord']), I, J, WALL_DIST):
				found = True
			else:
				# searching for a vertex close to the one I have chosen (in a circle with radium GRID_DISC), that is
				# sufficiently far from obstacles
				x = int(G_E.vs[POINTS[i]]['x_coord'])
				y = int(G_E.vs[POINTS[i]]['y_coord'])
				for vertex in G_E.vs:
					if vertex == G_E.vs[POINTS[i]]: pass
					x_ver = int(vertex['x_coord'])
					y_ver = int(vertex['y_coord'])
					if ((x_ver - x)**2 + (y_ver - y)**2 <= GRID_DISCRETIZATION**2)  and \
							all_free(y_ver, x_ver, I, J, WALL_DIST):
						G_E.vs[POINTS[i]]['x_coord'] = vertex['x_coord']
						G_E.vs[POINTS[i]]['y_coord'] = vertex['y_coord']
						found = True
						changed = True
						break
		...
		if not changed:
			if not large:
				...
				print 'WARNING: WALL_DIST too large. Change it, coordinates not fixable.'
		else:
			print 'NEW POINT ---> x_coord of node ' + str(i) + ": " + str(G_E.vs[POINTS[i]]['x_coord']) + \
				  ", y_coord of node " + str(i) + ": " + str(G_E.vs[POINTS[i]]['y_coord'])
			print 'OLD POINT ---> ' + str(old_pos)

		points_coords.append((G_E.vs[POINTS[i]]['x_coord'], G_E.vs[POINTS[i]]['y_coord']))
