...
#VERSION WITH INDEX OF GRAPH FROM 0 TO n_vertex-1 AND ROADMAP GRAPH BUILT AUTONOMOUSLY
N_ROBOT = 2

POINTS_TO_EXPLORE = [[0,4],[2,4],[2,3],[0,2],[0,3]] #needed to be in radiomap_graph dictionary
N_VERTEXES = 5
ROBOT_VELOCITY = 1
...
matrix_lenght = int(math.sqrt(D.size))
...
dict_time = {(i,j) : T[i][j] 
    for i in range(matrix_lenght) for j in range(i)}
