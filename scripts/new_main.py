...
#TODO STRATEGY: DORA/TIME
#TODO TIMEOUT

#################################################################################################################

# TODO PARSING SUL FILE PER LEGGERE LE CONFIGURAZIONI E DESTINAZIONI: IL PRIMO NUMERO DI OGNI COPPIA È IL PRIMO ROBOT, IL SECONDO È IL SECONDO ROBOT
...
class StrategyParams(object):
    def __init__(self, dora = None, min_time = None):
        self.dora = dora
...
class GenericRobot(object):
    def __init__(self, seed, robot_id, is_leader, sim, comm_range, map_filename, polling_signal_period, duration,
                 log_filename, comm_dataset_filename, teammates_id, n_robots, ref_dist, strategy, resize_factor,
        ...
        for i in xrange(n_robots):
            ...
            exec ("rospy.Subscriber('/robot_" + str(
                i) + "/amcl_pose', PoseWithCovarianceStamped, self.callback_pos_teammate" + str(
                i) + ", queue_size = 100)")
    ...
    def ask_new_signal_data_callback(self, event):
        ...
        for other_robot_id in xrange(self.n_robots):
            if other_robot_id == self.robot_id or not (
            self.comm_module.can_communicate(other_robot_id)): continue
            ...
            for other_robot_id_2 in xrange(self.n_robots):
                ...
                last_timestep = 0 if len(self.robot_data_list[other_robot_id_2]) == 0 else \
                self.robot_data_list[other_robot_id_2][-1].timestep
                try:  # could become disconnected on real robots
                    ...
                    rospy.loginfo(str(self.robot_id) + ' - failed service call while calling robot ' + str(
                        other_robot_id))
                    break
    ...
    def monitor_stop_motion_callback(self, event):
        ...
        elif ((self.stop_when_comm) or (not (self.is_leader) and self.teammate_arrived_nominal_dest and self.first_safe_pos_add)):
    ...
    def reset_stuff(self):
        ...
        #TODO STRATEGY DORA/MIN_TIME
        if (self.strategy != 'dora'):


        elif (self.strategy == 'dora'):
    ...
    def extrapolate_waypoints(self, poses):
        ...
        for i in xrange(1, len(poses)):
            dist += utils.eucl_dist((poses[i].pose.position.x, poses[i].pose.position.y),(poses[i - 1].pose.position.x, poses[i - 1].pose.position.y))
            if (dist >= meters):
    ...
    def poll_signal(self):
        ...
        for other_robot_id in xrange(self.n_robots):
            ...
            if (utils.eucl_dist((self.x, self.y),
                                self.other_robots_pos[other_robot_id]) > self.comm_range): continue
                ...
                if (abs(self.x - target[0]) < self.tol_dist and abs(self.y - target[1]) < self.tol_dist):
                    ...
                    if state == GoalStatus.PREEMPTED:
                        ...
                        # If trying to reconnect with teammate through backup plan...
                                  ...
                                  (self.strategy == 'multi2' and not (
                                  self.is_leader) and self.first_safe_pos_add and
...
class Leader(GenericRobot):
    def __init__(self, seed, robot_id, sim, comm_range, map_filename, polling_signal_period,
                 ...
                 env_filename, comm_dataset_filename, strategy, strategyParams, resize_factor, tiling,
                 errors_filename,
                 communication_model):
        ...
        #TODO STRATEGY DORA/MIN_TIME
        if (self.strategy == 'dora'):
            self.exploration_strategy = exploration_strategies.dora
        elif (self.strategy == 'min_time'):
            self.exploration_strategy = exploration_strategies.min_time
        ...
        def explore_comm_maps(self):
            if (self.strategy != 'dora'):
                self.explore_comm_maps_min_time()
            else:
                self.explore_comm_maps_dora()
        ...
        #TODO STRATEGY MIN_TIME
        def explore_comm_maps_min_time(self):

        def log_plan(self):
            f = open(self.log_filename, 'a')
            f.write('P ' + str((rospy.Time.now() - self.mission_start_time).secs) + ' ' + str(
                self.connect_first_attempt) + '\n')

        def aggregate_data(self):
            # add all the data.
            rospy.loginfo('Aggregating ' + str(len(self.signal_data)) + ' data of mine and ' + str(
                len(self.signal_data_follower)) + ' follower data.')
        ...
        #TODO STRATEGY MIN_TIME
        def send_myself_to_pair(self, dest):

        #TODO STRATEGY MIN_TIME
        def send_follower_to_pair(self, dest_follower, dest_leader):
...
class Follower(GenericRobot):
    ...
    def __init__(self, seed, robot_id, sim, comm_range, map_filename, polling_signal_period, duration,
        ...
        super(Follower, self).__init__(seed, robot_id, False, sim, comm_range, map_filename,
                                       polling_signal_period,
                                       duration, log_filename, comm_dataset_filename, teammates_id,
                                       n_robots, ref_dist, strategy, resize_factor, errors_filename)
    ...
    def execute_cb_min_time(self, goal):
